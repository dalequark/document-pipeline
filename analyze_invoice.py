...
# TODO: Conver to new Invoice API when it's availible in Python
...
def _analyze_invoice(bucket_name, filename):
    """ Given a gc location of a invoice file, extracts (when found)
...
def _insert_invoice_bigquery(filename, name, date, total, address, phone_number):
    client = bigquery.Client()
    table_id = os.environ["INVOICES_TABLE"]
...
def analyze_invoice(data, context):
    bucket = data["bucket"]
    name = data["name"]
    result = _analyze_invoice(bucket, name)
    print(result)
    if result:
        print("Inserting invoice gs://%s/%s into bigquery" % (bucket, name))
        _insert_invoice_bigquery(name, result["name"], result["date"], result["total"], result["address"], result["phone_number"])